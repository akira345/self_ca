...
# onlyで指定されたメソッドでset_csrメソッドが動く
  before_action :set_csr, only: %i[show edit update destroy download]
  ...
  def create
    ...
    respond_to do |format|
      if @csr.save
        ...
        logger.debug('-----------------------')
        # 各ファイルの出力先
        ca_params = Utils.generate_ca_param(@ca)
        cert_params = Utils.generate_cert_param(@csr)
        logger.debug('作成開始')
        cert = Makecert.new(ca_params)
        logger.debug('証明書生成')
        cert.create_cert(cert_params)
  ...
  def update
    ...
    logger.debug 'EDIT!!'
    # 変更前のホスト名を取得
    before_csr = Csr.select('hostname').where(id: params[:id]).where(user_id: current_user.id).first
    respond_to do |format|
  ...
  def set_csr
    ...
    @csr = Csr.where(id: params[:id]).where(user_id: current_user.id).first
