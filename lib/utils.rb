class Utils
  ...
  def self.delete_file(dirpath)
    if File.exist? dirpath
      FileUtils.rm_r(Dir.glob("#{dirpath}/"), secure: false)
  ...
  def self.generate_cert_param(csr)
    ...
    cert_param[:name] = [
      ['C', csr.country.to_s, OpenSSL::ASN1::PRINTABLESTRING],
      ['ST', csr.dn_st.to_s, OpenSSL::ASN1::UTF8STRING],
      ['L', csr.dn_l.to_s, OpenSSL::ASN1::UTF8STRING],
      ['O', csr.dn_o.to_s, OpenSSL::ASN1::UTF8STRING],
      ['OU', csr.dn_ou.to_s, OpenSSL::ASN1::UTF8STRING],
    ]
    cert_param
  end
  ...
  def self.generate_ca_param(ca)
    ...
    # CAのDN
    ca_param[:name] = [
      ['C', ca.country.to_s, OpenSSL::ASN1::PRINTABLESTRING],
      ['ST', ca.dn_st.to_s, OpenSSL::ASN1::UTF8STRING],
      ['L', ca.dn_l.to_s, OpenSSL::ASN1::UTF8STRING],
      ['O', ca.dn_o.to_s, OpenSSL::ASN1::UTF8STRING],
      ['OU', ca.dn_ou.to_s, OpenSSL::ASN1::UTF8STRING],
    ]
    # CAのホスト名情報
